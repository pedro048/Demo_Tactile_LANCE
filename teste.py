...
with PdfPages("cursor_trajectory_plots.pdf") as pdf:
    ...
    plt.figure()
    plt.plot(df["Time"], df["Speed"], color='red', label=r"$c_{\text{speed}}$")
    plt.xlabel(r"$t$ (s)")
    ...
    plt.figure()
    plt.plot(df["Time"], df["Frequency"], color='green', label=r"$f$")
    plt.xlabel(r"$t$ (s)")
    plt.ylabel(r"$f$ (Hz)")
    ...
    plt.figure()
    plt.plot(df["Time"], df["Duty Cycle"], color='green', label=r"$d$")
    plt.xlabel(r"$t$ (s)")
    plt.ylabel(r"$d$ (%)")
    ...
    # Outros gráficos para ID 2 e ID 3 similares...
